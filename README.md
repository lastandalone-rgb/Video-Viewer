...
### ⌨️ 單手操作導航 (Spatial Navigation)
- 支援使用鍵盤 `W`, `A`, `S`, `D` 進行全域空間導航（上下左右選取卡片或按鈕）
- 使用 `X` 鍵確認點擊，在播放器內可用來快速關閉影片
- **自動邊界捲動**：當焦點到達列表的最後一列時，自動平滑將捲軸推至最底（展現留白與分頁控制）；回到第一列時自動置頂
- **智慧區域隔離**：防止上下移動時焦點被側邊欄干擾，確保垂直導航能精準跳轉至分頁按鈕
- **智慧焦點記憶**：從影片播放器或進入/退出資料夾後，焦點會自動回到原先選取的影片卡片
- **無縫過濾失效元素**：自動忽略被停用 (disabled) 或不可見的控制項，讓導航更順暢
